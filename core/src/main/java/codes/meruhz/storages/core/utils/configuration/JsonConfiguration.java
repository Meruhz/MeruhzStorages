...
public class JsonConfiguration extends AbstractConfiguration<JsonElement> {
    ...
    @Override
    public void load() {
        @NotNull String format = new GsonBuilder().setPrettyPrinting().disableHtmlEscaping().create().toJson(super.getConfiguration());
        super.load();
        ...
        if(super.getFile().length() != 0) {
            ...
            try {
                configuration = JsonParser.parseString(AbstractConfiguration.getFileContent(super.getFile()));
